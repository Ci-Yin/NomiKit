 ...
 */
fun KoinAutoConfigurationScope.existInstance(vararg qualifiers: Qualifier) =
    koin.hasInstance(*qualifiers)

inline fun KoinAutoConfigurationScope.onExistInstance(
 ...
 * @param qualifiers 需要检查的限定符
 */
fun KoinAutoConfigurationScope.missInstances(vararg qualifiers: Qualifier) =
    !existInstance(*qualifiers)

inline fun KoinAutoConfigurationScope.onMissInstances(
    vararg qualifiers: Qualifier,
    then: () -> Unit
) {
 ...
 * @param keys 需要检查的属性键
 */
fun KoinAutoConfigurationScope.existProperties(vararg keys: String): Boolean =
    koin.hasProperties(*keys)
 ...
 * @param keys 需要检查的属性键
 */
fun KoinAutoConfigurationScope.missProperties(vararg keys: String): Boolean =
    !existProperties(*keys)
 ...
 */
fun KoinAutoConfigurationScope.eqProperty(key: String, value: Any): Boolean =
    koin.eqProperty(key, value)
