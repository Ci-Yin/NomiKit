...
本指南将详细介绍如何为第三方库添加 KoinBoot 自动装配支持，使其能够与 KoinBoot 框架无缝集成。通过本指南，您将了解
KoinBoot
的实现原理、核心组件以及如何适配新的第三方库。
...
每个支持自动装配的库都会提供一个 `KoinBootInitializer`，用于注册其自动配置或生命周期扩展器。
`KoinBootInitializer` 是连接库和
...
2. **使用 KoinBootLifecycleExtender**：适用于需要在特定生命周期阶段执行初始化或清理的库，如
   Sentry、Kermit
...
2. **MultiplatformSettingsAutoConfiguration**：使用 koinAutoConfiguration 创建，配置 Settings 和
   Settings.Factory
...
DataStore 使用 KoinAutoConfiguration 方式适配，支持类型安全的数据存储和 kotlinx-serialization：

1. **DataStoreProperties**：定义了 DataStore 的配置选项，如存储目录
2. **DataStoreAutoConfiguration**：使用 koinAutoConfiguration 创建，配置 FileSystem、DataStorePath 和
   DataStoreFactory
3. **DataStoreBootInitializer**：注册 DataStoreAutoConfiguration
...
1. 提供了 `DataStoreFactory` 用于创建类型安全的 DataStore 实例
2. 提供了 `JsonDataStoreSerializer` 基于 kotlinx-serialization 的序列化器
3. 使用 `onMissInstances` 检查用户是否已经配置了 FileSystem、DataStorePath 等组件
4. 支持多平台（Android/iOS/Desktop），通过 expect/actual 处理平台特异性路径获取
5. 支持用户自定义存储目录
...
// 在 Koinboot 初始化时添加 DataStore 模块
val koin = runKoinBoot {
    DataStoreBootInitializer(this)
}

// 使用 DataStoreFactory 创建 DataStore
val factory: DataStoreFactory = koin.get()
val dataStore = factory.create(
...
5. **遵循命名约定**：使用一致的命名约定，如 `<库名>Properties`、`<库名>AutoConfiguration`、
   `<库名>Extender`、
   `<库名>BootInitializer`
